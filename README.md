...
- **`Rejected`** — бронь обработана, но отклонена (например, из-за конфликта с уже существующей бронью на то же событие или иного бизнес-правила).
...
4. Фоновый сервис `BookingProcessingBackgroundService` (реализует `BackgroundService`) асинхронно забирает брони из очереди, выполняет проверку бизнес-правил и обновляет статус брони на `Confirmed` или `Rejected`, заполняя `processedAt`.
...
sleep 10
curl -i http://localhost:5102/bookings/BOOKING_ID
# → 200 OK, status: "Confirmed" (или "Rejected"), processedAt заполнен
\`\`\`

Тот же сценарий можно выполнить через Swagger UI (`/swagger`): последовательно вызвать `POST /events`, `POST /events/{id}/book` и несколько раз `GET /bookings/{id}` с задержкой, наблюдая переход статуса из `Pending` в `Confirmed`/`Rejected`.
